...
export function getMessages( {state,commit}, payload ){
  console.log('getMessages',  payload) // ex) createRoom {user1: "skk7541", user2: "kkk7541"}
  // axios.get("/chat/room",{id:payload.chatRoomId}
  axios.get("/chat/room"+"/"+payload.id
    ,{
    headers:{
      Authorization:"Bearer "+state.accessToken
    }
  ...
  .then(res=>{
    console.log(res.data);
    commit('SET_MESSAGE_EX', res.data);
  })
  .catch(err=>{
    console.log(err)
    return err
